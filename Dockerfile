...
## libevent
RUN cd /usr/local/src && curl -o https://github.com/downloads/libevent/libevent/libevent-2.0.21-stable.tar.gz
...
## tmux
RUN cd /usr/local/src && curl -o http://downloads.sourceforge.net/tmux/tmux-1.9a.tar.gz
RUN cd /usr/local/src && tar -xvf tmux-1.9a.tar.gz && cd tmux-1.9a && ./configure && make && make install

## eclipse
RUN cd /usr/local/src && curl -o http://ftp.jaist.ac.jp/pub/eclipse/technology/epp/downloads/release/luna/SR1/eclipse-java-luna-SR1-linux-gtk-x86_64.tar.gz
...
RUN cd /opt/eclim && curl -O http://jaist.dl.sourceforge.net/project/eclim/eclim/2.3.2/eclim_2.3.2.jar
