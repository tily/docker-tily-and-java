...
RUN yum install -y tar && cd /usr/local/src && tar -xvf libevent-2.0.21-stable.tar.gz && cd libevent-2.0.21-stable && ./configure && make && make install
...
#RUN cd /opt/eclim && curl -LO http://jaist.dl.sourceforge.net/project/eclim/eclim/2.4.0/eclim_2.4.0.jar
#RUN sed -i -e 's/\s*Defaults\s*requiretty$/#Defaults    requiretty/' /etc/sudoers
#RUN sudo -u tily java -Dvim.files=/home/tily/.vim -Declipse.home=/opt/eclipse -jar /opt/eclim/eclim_2.4.0.jar install
